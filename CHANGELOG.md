...
- **`server.js:678` — Mistral endpoint (no-op clarification)** — Mistral URL already includes `/v1` (`https://api.mistral.ai/v1`), so `${MISTRAL_URL}/chat/completions` resolves to the correct `https://api.mistral.ai/v1/chat/completions`. The original code was correct; no functional change made.
